...
function Channel() {
    ...
    useEffect(() => {
        const ChannelFetch = async () => {
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL}/channel/${channelId}`);
                if (!response.ok) {
    ...
    //     try {
    //         const videoData = await fetch(`${import.meta.env.VITE_API_URL}/video/${videoid}`).then(data => data.json())
