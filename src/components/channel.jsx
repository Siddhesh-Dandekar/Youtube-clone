...
function Channel() {
    ...
    useEffect(() => {
        const ChannelFetch = async () => {
            try {
                const response = await fetch(`https://youtube-clone-api-seven.vercel.app/channel/${channelId}`);
                if (!response.ok) {
    ...
    //     try {
    //         const videoData = await fetch(`https://youtube-clone-api-seven.vercel.app/video/${videoid}`).then(data => data.json())
